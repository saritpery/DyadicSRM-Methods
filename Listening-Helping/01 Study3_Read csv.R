...
# Read The Qualtics exported data:
# ================================
g.s.num <- 3  # Number of different group sizes available!!!!
group.size <- c(4,5,6) #group sizes

INPUT <- list()
INPUT[[1]] <- read.csv("SRM+-+Study+3+-+4+participants_December+6,+2022_10.14.csv", stringsAsFactors = FALSE)
INPUT[[2]] <- read.csv("SRM+-+Study+3+-+5+participants_December+6,+2022_10.15.csv", stringsAsFactors = FALSE)
INPUT[[3]] <- read.csv("SRM+-+Study+3+-+6+participants_December+6,+2022_10.16.csv", stringsAsFactors = FALSE)
