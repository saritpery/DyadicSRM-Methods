...
# In this file:   ==============================================================
# 2-levels analyses using nlme with Kenny's dummy-codes. 
# 1. The dyadic hypothesis for humility-creativity-contrib-safety
#    As appears in the JAP analyses.
# 2. Addition of ind-level interactions 
# 3. Compare and slightly look into ML vs REML
# 4. Comparison of models with vs. w/o ind-levels
# 5. Humility power-couples index
# 6. Review analyses for the dyadic raw scores vs. the decomposed dyadic scores
# 7. Look into scaled vs. non-scaled scores
# 8. The ind-level equivalent models

# Main findings:  ==============================================================
# 1. Adding the targets interaction improves models fit!
# 2. Power couples matter. 
#    Power couples vs. target scores interaction don't change much. 
#    That still requires further investigation. 
# 3. Perceptions interaction don't affect raw/decomposed dyadic effects!
# 4. 
...
if(dontSkip){
  ...
  fit <- showAPIM("Creat_rel.BA ~ Hum_rel.AB * Hum_rel.BA + 
                  ...
                  aDum + pDum")
                  # result: no dyad intrction eff -0.12 p=.007
  fit <- showAPIM("Creat_rel.BA ~ Hum_rel.AB  + Hum_rel.BA + 
                  Hum_perc.A:Hum_perc.B + Hum_target.A:Hum_target.B +
                  aDum + pDum")
                  # result: the percs' interction don't contribute to the model.
                  #         value=-.01 p=.92
  fit <- showAPIM("Creat_rel.BA ~ Hum_rel.AB * Hum_rel.BA + 
                  Hum_target.A:Hum_target.B + aDum + pDum")
                  # result: no dyad intrction eff -.11 p=.08
  fit1 <- showAPIM("Creat_rel.BA ~ Hum_rel.AB + Hum_rel.BA + 
                  Hum_target.A:Hum_target.B + aDum + pDum")
  # Add target creativity
  fit <- showAPIM("Creat_rel.BA ~ Hum_rel.AB + Hum_rel.BA + 
                  Hum_target.A:Hum_target.B + 
                  Creat_target.A:Creat_target.B + aDum + pDum")
                  # result: no creativity targ intrction eff -.06 p=.12
  # Add target creativity to raw scores
  fit <- showAPIM("Creat_raw.BA ~ Hum_rel.AB + Hum_rel.BA + 
                  Hum_target.A:Hum_target.B + 
                  Creat_target.A:Creat_target.B + aDum + pDum")
                  # result: no creativity targ intrction eff -.02 p=.36
