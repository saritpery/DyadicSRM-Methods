...
# my addition: transform data into a long format 
# ============================================================================
    # The goal is to compare the outcome to the tripleR package
    rdata <- data.frame(t(matrix(data, nrow = nact)))
    ps <- paste0("id", 1:nact)
    names(rdata) <- ps
    rdata <- mutate(rdata, actor = ps) %>% select(actor, everything())
    longrdata <- pivot_longer(rdata, cols = all_of(ps), names_to = "partner", 
                              values_to = "v") %>% 
      as.data.frame()
    # my addition: calculate effect sizes using tripleR
    fit <- RR(v ~ actor*partner, data = longrdata)
    trWKS <- select(fit$effects, -v.s)
    trWKSrel <- fit$effectsRel
    trWKSVcov <- fit$varComp
    rownames(trWKSVcov) <- NULL
  

# turn data data into a matrix, and transpose it 
dim(data)<-c(nact,nact)
data<-t(data) 
ntot<-nact*(nact-1)  # ntot = number of actual ratings
ndy<-ntot/2
neff<-2*nact+ntot  # actor effects + partner effects + dyadic effects
dv<-rep(0,ntot)
k<-0
for (i in 1:nact) {
  for (j in i:nact) {
...
Act<-rep(0,ntot)
Par<-rep(0,ntot)
ac<-matrix(0,ntot,nact) # indicator matrices for actor and partner
pa<-matrix(0,ntot,nact)
k<-0
for (i in 1:nact) {
  for (j in i:nact) {
    if (i!=j)
...
for (i in 1:nact) {
  Z<-matrix(0,ntot,neff) # indicator matrix for each eﬀect (col) in each data point (row)
  j<-2*(i-1)+1
  k<-2*i
  Z[,j]<-ac[,i]
  Z[,k]<-pa[,i] }
