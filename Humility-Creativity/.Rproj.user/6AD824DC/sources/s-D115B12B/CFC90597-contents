...
# Report demographic balances:
  t <- select(x, a.id, Lang, Gender, Age) %>% distinct()
  n <- nrow(t)
  # Lang = 1 - People who filled an English survey
  # Lang = 2 - People who filled a Hebrew survey
  # percent of people who filled the survey in English:
  sum(t$Lang == 1) / n      # 14% filled the survey in English
  # Gender = 1 - Female
  # Gender = 2 - Male
  sum(t$Gender == 1) / n    # 72% Females
  mean(t$Age, na.rm = TRUE) # average age is 39.8
  sd(t$Age, na.rm = TRUE)   # sd of age is 11.15
